...
export default function DetailsScreen({ route }) {
  ...
  const handleSaveInterest = async () => {
    try {
      ...
      Alert.alert(
        "Sucesso!",
        `${attraction.name} foi adicionada aos seus interesses!`,
      );
    } catch (error) {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.buttonsContainer}>
          ...
          <TouchableOpacity
            style={btn.base}
            activeOpacity={0.8}
            onPress={() => Linking.openURL(`mailto:${attraction.email}`)}
          >
        ...
        {attraction.requiresReservation && (
          <View style={styles.reservationBox}>
            <Text style={styles.reservationText}>
              Este local possui capacidade limitada.
            </Text>
            <TouchableOpacity
              style={btn.base}
              activeOpacity={0.8}
              onPress={() => {
                const mensagem = `Olá! Gostaria de verificar a disponibilidade e fazer uma reserva para: ${attraction.name}.`;
                Linking.openURL(
                  `whatsapp://send?phone=${attraction.phone}&text=${encodeURI(mensagem)}`,
                );
              }}
            >
              <Text style={btn.text}>Reservar via WhatsApp</Text>
            </TouchableOpacity>
          </View>
        )}
